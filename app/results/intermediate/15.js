...
function getsales(results) {
    ...
    switch (tableid) {
        case "15.1.":
            ...
            <a class='pointer' onclick='getDataDetail(\"" + bothCurr[i] + "\", \"salescurrency\", this)' id='detail_" + bothCurr[i] + "> \
            ...
            if (ctdata) {
                for (var i = startYear; i <= endYear; i++) {
                    ...
                    for (var j = 0; j < ctdata.length; j++) {
                        ...
                        data['R_' + ctdata[j]['ClientName']] = checkval(ctdata[j]['R_' + ctdata[j]['TradeCurrency'] + '_' + i]);
                    }
                    datar.push(data);
                }
            }
        ...
        case "15.2.":
            ...
            for (var i = 0; i < producttypes.length; i++) {
                ...
                controls += "<li role='presentation' class='" + active + "'> \
                <a class='pointer' onclick='getDataDetail(\"" + producttypes[i]['id'] + "\", \"salesproduct\", this)' id='detail_" + producttypes[i]['id'] + "> \
...
function showData(results) {
        ...
        case "15.1.":
            if (ctdata) {
                for (var j = 0; j < ctdata.length; j++) {
                    ...
                    cols.push({
                        name: 'R_' + ctdata[j]['ClientName'],
                        columngroup: ctdata[j]['ClientName'] + ' - ' + productName,
                        map: 'R_' + ctdata[j]['ClientName'],
                        text: 'Revenues (Million)'
                    });
                }
            }
            //console.log(cols);
    ...
    CreateGrid(cols, getsales(results), columngroups);
}